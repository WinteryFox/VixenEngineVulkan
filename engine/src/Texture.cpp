...
namespace Vixen {
    Texture::Texture(const std::unique_ptr<LogicalDevice> &logicalDevice,
        ...
        createBuffer(logicalDevice, size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY, stagingBuffer,
                     stagingAllocation);
        ...
        createImage(logicalDevice, physicalDevice, width, height, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
                    VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, texture, allocation);
        transitionImageLayout(logicalDevice, texture, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED,
                              VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        copyBufferToImage(logicalDevice, stagingBuffer, texture, static_cast<uint32_t>(width),
                          static_cast<uint32_t>(height));
        transitionImageLayout(logicalDevice, texture, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        ...
        if (imageViewCreateResult != VK_SUCCESS)
            error("Failed to create texture view for texture " + absolutePath + " failed with result: " +
