...
const float g = -0.9;
const float g2 = g * g;
const vec3 sunPosition = normalize(vec3(0.0, 0.0, -1.0));
...
void main() {
    ...
    vec3 D = mat3(projection) * mat3(view) * normalize(vec3(-uv, -(tan(radians(60.0)))));

    float sR = rayleighAtt / uv.y;
    float sM = mieAtt / uv.y;

    float cosine = clamp(dot(D, sunPosition), 0.0, 1.0);
